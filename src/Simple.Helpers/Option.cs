...
public static partial class Option
{
    ...
    public static IOption<T> Try<T>(Func<T> get)
    {
        try
        {
            return Value(get());
        ...
        {
            return Error<T>(ex, () => "Option.Try()");
        }
    }
    public static IOption<R> Try<T, R>(T arg, Func<T, R> select)
        => Try(() => select(arg));

    public static IOption<T> Value<T>(T value)
        => new Option<T>(value);

    public static IOption<string> String(string? value)
        => string.IsNullOrEmpty(value) ? Empty<string>() : Value<string>(value!);
    ...
    /// <summary> make message as $"{methodName}({methodArgs}) {methodResult}" </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static string LogMessageMethod<T>(Func<string?> methodArgs, Func<string?>? methodResult = null, [CallerMemberName] string? methodName = null)
